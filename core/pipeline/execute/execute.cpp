 ...
 * @LastEditors: 苗金标
 * @LastEditTime: 2023-05-23 15:50:12
 * @Description: 
 */
#include "execute.h"
#include "alu.h"
        ...
        if(!(wb_feedback_pack_t.enable && wb_feedback_pack_t.flush)){
            ...
            if(!issue_execute_fifo->is_empty() && !execute_wb_fifo->is_full()){
                ...
                if(instInfo.valid){
                    ...
                    if(bru_feedback_pack_t.enable && bru_feedback_pack_t.jump){
                        instInfo.debug_pc = bru_feedback_pack_t.next_pc;
                    }else{
                        instInfo.debug_pc = instInfo.pc + 4;
                    }
                    if(!instInfo.valid){
                        instInfo.has_execp = true;
                        instInfo.execp_id = exc_illegal_instr;
